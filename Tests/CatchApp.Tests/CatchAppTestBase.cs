    ...
    public abstract class CatchAppTestBase : AbpIntegratedTestBase
    {
        ...
        protected CatchAppTestBase()
        {
            ...
            UsingDbContext(context =>
            {
                ...
                new DefaultMemberBuilder(context).BuildTestMembers();
                new DefaultTestVisitBuilder(context).Build();
            });
        ...
        protected void UsingDbContext(Action<CatchAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<CatchAppDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CatchAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CatchAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CatchAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CatchAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<CatchAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CatchAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CatchAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CatchAppDbContext, Task<T>> func)
        {
            T result;
